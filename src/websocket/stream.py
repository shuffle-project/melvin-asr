...
class Stream:
            ...
            while not self.close_stream:
                ...
                if "bytes" in message:
                    ...
                    if len(self.chunk_cache) >= self.final_treshold:
                        self.logger.debug(
                            f"NEW FINAL: length of chunk cache: {
                                len(self.chunk_cache)}"
                        )
                        asyncio.ensure_future(
                            self.transcribe_all_chunk_cache(
                                websocket, self.chunk_cache, self.recently_added_chunk_cache
                            )
                        )
                        self.recently_added_chunk_cache = b""
                        self.chunk_cache = b""
                    ...
                    if len(self.recently_added_chunk_cache) >= self.partial_treshold:
                        self.logger.info(
                            f"NEW PARTIAL: length of chunk cache: {
                    ...
                    self.logger.debug(
                        f"Received control message (string): {message}")
                    if "eof" in message:
                        if "eof-finalize" in message:
                            ...
                            await self.transcribe_all_chunk_cache(
                                websocket, self.chunk_cache, self.recently_added_chunk_cache, skip_send=True
                            )
    ...
    async def transcribe_all_chunk_cache(
        ...
        try:
            ...
            data = self.transcriber._transcribe(
                ...
                self.last_final_result_object["text"],
            )
            self.adjust_threshold_on_latency()
            overall_transcribed_seconds = len(
                self.overall_transcribed_bytes) / BYTES_PER_SECOND
            last_final_bytes_seconds = len(
                self.last_final_bytes) / BYTES_PER_SECOND
            ...
            if "segments" in data:
                ...
                result = {"result": words, "text": " ".join(
                    [x["word"] for x in words])}
            ...
            self.logger.debug(f"Final Transcription took {
                              time.time() - start_time:.2f} s")

        except Exception:
            self.logger.error(f"Error while transcribing audio: {
                              traceback.format_exc()}")
            self.close_stream = True
    ...
    async def transcribe_chunk_partial(self, websocket, chunk_cache, recent_cache):
        try:
            ...
            if len(chunk_cache) == 0:
                self.logger.warning(
                    "Received empty chunk, skipping transcription.")
    ...
    def adjust_threshold_on_latency(self):
        ...
        if seconds_difference > FINAL_TRANSCRIPTION_TIMEOUT - 2:
            ...
            self.partial_treshold = max(
                self.partial_treshold * 0.75, BYTES_PER_SECOND * PARTIAL_TRANSCRIPTION_TIMEOUT)
    ...
    def concatenate_audio_with_crossfade(self, audio_chunk1: bytes, audio_chunk2: bytes, crossfade_duration=20) -> bytes:
        """
        ...
        if len(segment1) < crossfade_duration or len(segment2) < crossfade_duration:
            crossfade_duration = min(
                len(segment1), len(segment2), crossfade_duration)
